    ...
    def get_data(self, download=False, unzip=False, force_rebuild=False):
        ...
        if unzip:                       
            for subset in ['train', 'test']:
                ...
                for source_file in image_filepath_list:
                    datatype = 'image'
...
class Fashionpedia(GetDataset):
    ...
    def scale_segmentation(self, segmentation_list):
        new_list = []
        for seg in segmentation_list:
            
            if isinstance(seg, list):
                seg = seg[0]
                new_list.append([seg[i] / image_width if i%2==0 else seg[i] / image_height 
                                for i in range(len(seg))])
            
            else:  # skip masks ####### update this step
                new_list.append([])
        return new_list
    ...
    def json_to_dataframe(self, filename, save_subset):
        ...
        for id_num in all_ids:
            ...
            """  need to imporove function to handle 'rle' format entries
            # normalize segmentation values into [0,1]
            segmentation_list = self.scale_segmentation(segmentation_list)
            """
